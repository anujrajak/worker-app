...
$proRec = "SELECT * FROM `billproducts` bp INNER JOIN product p ON bp.bpPid=p.proId INNER JOIN category c ON p.proCat=c.catId INNER JOIN size s ON s.sizeId=p.proSize WHERE `bpBid` = '". $row['billUid'] ."'";
...
<body>
		...
		<div class="container-fluid d-flex justify-content-between">
			<div class="col-lg-3 pl-0">
				...
				<p>Order Date: <b><?php echo $row['billDate']; ?></b><br>Receive Date: <b><?php echo $row['billRecDate']; ?></b></p>
			</div>
				...
				<table class="table table-bordered">
					...
					<tbody>
						<?php $s=0; foreach ($proRecs as $pro) { $s++; ?>
							<tr>
								<td><?php echo $s; ?></td>
								<td><?php echo ucwords($pro['catName']) ?></td>
								<td><?php echo ucwords($pro['sizeName']) ?></td>
